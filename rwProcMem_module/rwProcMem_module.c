...
// 全局变量
static uid_t g_manager_uid = -1; // 管理器UID
static struct list_head g_authorized_apps; // 授权应用列表
static DEFINE_MUTEX(auth_mutex); // 授权列表互斥锁
...
// 初始化授权列表
static void init_authorization_list(void) {
    INIT_LIST_HEAD(&g_authorized_apps);
}

// 注册管理器命令处理
static ssize_t OnCmdRegisterManager(struct ioctl_request *hdr, char __user* buf) {
    char secret_key[256];
    struct task_struct *task = x_get_current();
    uid_t uid = task_uid(task).val;
    
    // 验证密钥
    if (x_copy_from_user(secret_key, buf, sizeof(secret_key))) {
        return -EFAULT;
    }
    
    if (strcmp(secret_key, CONFIG_MANAGER_SECRET_KEY) != 0) {
        return -EACCES; // 密钥错误
    }
    
    // 注册为管理器
    g_manager_uid = uid;
    printk(KERN_INFO "Manager registered: UID=%d\n", uid);
    return 0;
}

// 授权应用命令处理
static ssize_t OnCmdAuthorizeApp(struct ioctl_request *hdr, char __user* buf) {
    struct app_authorization auth;
    struct authorized_app *app;
    
    // 检查调用者是否是管理器
    if (task_uid(x_get_current()).val != g_manager_uid) {
        return -EPERM; // 无权限
    }
    
    // 复制授权数据
    if (x_copy_from_user(&auth, buf, sizeof(auth))) {
        return -EFAULT;
    }
    
    // 创建新授权项
    app = kzalloc(sizeof(*app), GFP_KERNEL);
    if (!app) return -ENOMEM;
    
    app->uid = auth.uid;
    strncpy(app->package, auth.package, sizeof(app->package));
    app->expiration = auth.expiration;
    
    // 添加到授权列表
    mutex_lock(&auth_mutex);
    list_add_tail(&app->list, &g_authorized_apps);
    mutex_unlock(&auth_mutex);
    
    printk(KERN_INFO "App authorized: %s (UID=%d)\n", app->package, app->uid);
    return 0;
}

// 检查授权状态
static bool is_app_authorized(uid_t uid) {
    struct authorized_app *app;
    bool authorized = false;
    uint64_t now = ktime_get_real_seconds();
    
    mutex_lock(&auth_mutex);
    list_for_each_entry(app, &g_authorized_apps, list) {
        if (app->uid == uid && app->expiration > now) {
            authorized = true;
            break;
        }
    }
...
static ssize_t rwProcMem_read(struct file* filp,
    ...
    // 白名单命令：注册、授权、检查状态
    if (hdr.cmd != CMD_REGISTER_MANAGER && 
        hdr.cmd != CMD_AUTHORIZE_APP &&
        hdr.cmd != CMD_REVOKE_APP &&
        hdr.cmd != CMD_CHECK_AUTHORIZATION) {
        
        // 检查是否是管理器或授权应用
        if (uid != g_manager_uid && !is_app_authorized(uid)) {
            printk(KERN_WARNING "Unauthorized access from UID: %d\n", uid);
            return -EACCES;
        }
    }
