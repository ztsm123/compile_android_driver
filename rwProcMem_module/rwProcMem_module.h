...
#define CONFIG_AUTH_KEY "luoyaonb666" // 通信认证密钥
...
enum {
	...
	CMD_REGISTER_MANAGER,          // 注册管理器
    CMD_AUTHORIZE_APP,             // 授权应用
    CMD_REVOKE_APP,                // 撤销授权
    CMD_CHECK_AUTHORIZATION,       // 检查授权状态
};

// 授权信息结构
struct app_authorization {
    uid_t uid;                     // 应用UID
    char package[256];             // 应用包名
    uint64_t expiration;           // 授权过期时间（时间戳）
};
